...
train_pipeline = [
    ...
    dict(type='MVAPasteBirds', minW=5, maxW=80, num_range_per_image=[0, 5]),
    dict(
...
data = dict(
    ...
    train=dict(
        type='DroneDataset',
        ann_file=
        '/root/Document/MVA2023SmallObjectDetection4SpottingBirds/data/mva2023_sod4bird_train/annotations/split_train_coco.json',
        img_prefix=
        '/root/Document/MVA2023SmallObjectDetection4SpottingBirds/data/mva2023_sod4bird_train/images/',
        pipeline=[
            dict(
    ...
    val=dict(
        type='DroneDataset',
        ann_file=
        '/root/Document/MVA2023SmallObjectDetection4SpottingBirds/data/mva2023_sod4bird_train/annotations/split_val_coco.json',
        img_prefix=
        '/root/Document/MVA2023SmallObjectDetection4SpottingBirds/data/mva2023_sod4bird_train/images/',
        pipeline=[
            dict(type='LoadImageFromFile', to_float32=True),
            dict(type='LoadAnnotations', with_bbox=True),   # demo 展示用
            dict(
                type='MultiScaleFlipAug',
                scale_factor=1.0,
                flip=False,
                transforms=[
                    ...
                    dict(
                        ...
                        meta_keys=('filename', 'ori_shape', 'img_shape',
                                   'scale_factor', 'flip', 'flip_direction',
                                   'img_norm_cfg'),
                        keys=['img', 'gt_bboxes', 'gt_labels', 'ori_filename']) # demo 顯示用
                        # keys=['img'])
                ])
        ]),
    test=dict(
        ...
        pipeline=[
            ...
            dict(
                ...
                scale_factor=1.0,
                flip=False,
                # img_scale=[(800, 800), (1600, 1600), (3840, 2160)],  # 多尺度設定
                # flip=True,
                transforms=[
                    dict(type='Resize', keep_ratio=True),
                    dict(type='RandomFlip'),
...
work_dir = './work_dirs/cascade_mask_rcnn_swin_finetune_rfla_4stage'
auto_resume = False
gpu_ids = range(0, 1)
