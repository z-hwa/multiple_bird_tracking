...
class KalmanFilterNew_score_new(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x, dim_z, dim_u=0, args=None):
        ...
        self.innovation_window = 50  # 滑動窗口大小，可以根據需求調整
        self.adaptive_R = True      # 啟用 R 的自適應調整
        self.R_alpha = 0.9          # 遺忘因子，用於平滑 R 的估計
        # breakpoint()
    ...
    def update(self, z, R=None, H=None):
        ...
        # 保存新息
        self.innovation_history.append(self.y)
        if len(self.innovation_history) > self.innovation_window:
            self.innovation_history.pop(0)

        if self.adaptive_R and len(self.innovation_history) > 1:
            # 估計新息的樣本協方差
            innovation_covariance = np.cov(np.hstack(self.innovation_history), rowvar=True, bias=True)

            # breakpoint()
            # 使用遺忘因子平滑 R 的估計
            self.R = self.R_alpha * self.R + (1 - self.R_alpha) * (innovation_covariance - dot(dot(H, self.P_prior), H.T))

            # 確保 R 的對角線元素非負
            self.R = np.diag(np.maximum(np.diag(self.R), 1e-6)) # 避免 R 的對角線過小
