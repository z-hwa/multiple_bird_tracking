...
def median_trick(X):
    ...
    N = len(X)
    if N == 0:
        return -1  # 返回一個預設值，例如 1.0，或者根據你的需求設定
...
def gp_interpolation(txt_path, save_path, reference_dir, n_min=30, n_dti=20):
    ...
    for seq_txt in seq_txts:
        ...
        for track_id in range(min_id, max_id + 1):
            ...
            l = 1000.0 / n_frame
            kernel = 20 * RBF(length_scale=20, length_scale_bounds=(1e-10, 1e5))
            ...
            if n_frame > n_min:
                ...
                for frame in frames:
                    if frame not in to_fill_frames:
                        ...
                        curr_bbox = np.array([[curr_x[0, 0] - 0.5 * width, curr_y[0, 0] - 0.5 * height,
