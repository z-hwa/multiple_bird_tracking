...
# pub test phase 2
output_dir = "HybridSORT/phase_2/best_TCM_reverse"  # 你可以自訂輸出資料夾
predictions_json_path = "../mva2023/" + "work_dirs/cascade_mask_rcnn_swin_finetune_rfla_4stage/results_smot4sb_phase2.json"
annotations_json_path = "/root/Document/data/MVA2025/annotations/test_coco.json"

# val
# output_dir = "Cascade_outputs/val/best_reverse"  # 你可以自訂輸出資料夾
# predictions_json_path = "../mva2023/" + "best_result/results_smot4sb_val.bbox.json"
...
def predict_videos(res_folder, args):
    ...
    for video_name, files in video_image_dict.items():
        ...
        tracker = Hybrid_Sort(args=args, det_thresh=args.track_thresh, iou_threshold=0.1, use_byte=args.use_byte,
                        asso_func=asso_func, inertia=inertia)
        results = []

        # 模擬倒序的 frame_id
        total_frames = len(files)
        for reversed_frame_id, img_path in enumerate(reversed_files, 1):
            # 注意：這裡的 frame_id 是相對於原始順序的倒序
            frame_id = total_frames - reversed_frame_id + 1
