...
class SO_HOTA(_BaseMetric):
    """Class which implements the SO-HOTA metrics.
    See: https://mva-org.jp/mva2025/challenge
    """
    ...
    def __init__(self, config=None):
        ...
        self.integer_array_fields = ['SO-HOTA_TP', 'SO-HOTA_FN', 'SO-HOTA_FP']
        self.float_array_fields = ['SO-HOTA', 'SO-DetA', 'SO-AssA', 'SO-DetRe', 'SO-DetPr', 'SO-AssRe', 'SO-AssPr', 'SO-LocA', 'SO-OWTA']
        self.float_fields = ['SO-HOTA(0)', 'SO-LocA(0)', 'SO-HOTALocA(0)']
        self.fields = self.float_array_fields + self.integer_array_fields + self.float_fields
        self.summary_fields = self.float_array_fields + self.float_fields

    @_timing.time
    def eval_sequence(self, data):
        """Calculates the SO-HOTA metrics for one sequence"""
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            if len(gt_ids_t) == 0:
                for a, alpha in enumerate(self.array_labels):
                    res['SO-HOTA_FP'][a] += len(tracker_ids_t)
                continue
            if len(tracker_ids_t) == 0:
                for a, alpha in enumerate(self.array_labels):
                    res['SO-HOTA_FN'][a] += len(gt_ids_t)
                continue

            # Get matching scores between pairs of dets for optimizing SO-HOTA
            similarity = data['similarity_scores'][t]
            ...
            for a, alpha in enumerate(self.array_labels):
                ...
                res['SO-HOTA_TP'][a] += num_matches
                res['SO-HOTA_FN'][a] += len(gt_ids_t) - num_matches
                res['SO-HOTA_FP'][a] += len(tracker_ids_t) - num_matches
                if num_matches > 0:
                    res['SO-LocA'][a] += sum(similarity[alpha_match_rows, alpha_match_cols])
        ...
        for a, alpha in enumerate(self.array_labels):
            ...
            res['SO-AssA'][a] = np.sum(matches_count * ass_a) / np.maximum(1, res['SO-HOTA_TP'][a])
            ass_re = matches_count / np.maximum(1, gt_id_count)
            res['SO-AssRe'][a] = np.sum(matches_count * ass_re) / np.maximum(1, res['SO-HOTA_TP'][a])
            ...
            if ignore_empty_classes:
                res[field] = self._combine_sum(
                    {k: v for k, v in all_res.items()
                     if (v['SO-HOTA_TP'] + v['SO-HOTA_FN'] + v['SO-HOTA_FP'] > 0 + np.finfo('float').eps).any()}, field)
            else:
        ...
        for field in self.float_fields + self.float_array_fields:
            if ignore_empty_classes:
                res[field] = np.mean([v[field] for v in all_res.values() if
                                      (v['SO-HOTA_TP'] + v['SO-HOTA_FN'] + v['SO-HOTA_FP'] > 0 + np.finfo('float').eps).any()],
                                     axis=0)
            else:
